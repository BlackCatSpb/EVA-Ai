...
- Класс: `CoreBrain`
- Новые методы:
  - `clear_all_contradictions() -> Dict[str, Any]`
    - Безопасная обертка, делегирующая очистку противоречий в `contradiction_resolver`.
    - Не инициирует тяжелые загрузки ML-моделей.
    - Возвращает отчет: `{ "ok": bool, "cleared": int, "db_path": Optional[str], "error": Optional[str] }`.
  - Поведение при hot-reload:
    - `soft_reload()` автоматически вызывает `clear_all_contradictions()` для предотвращения «застывших» состояний.
...
- Файл: `cogniflex/mlearning/training_orchestrator.py`
- Класс: `TrainingOrchestrator`
- Важное поведение:
  - В начале `train_from_document(...)` выполняется безопасная очистка противоречий через `brain.clear_all_contradictions()` (если доступно). Это гарантирует, что обучение стартует без устаревших состояний.
  - Метрика `training.contradictions_cleared_pre` фиксирует количество очищенных записей перед обучением.
