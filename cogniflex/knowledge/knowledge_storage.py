"""
Модуль хранения графа знаний для CogniFlex
...
logger = logging.getLogger("cogniflex.knowledge_storage")
...
class KnowledgeStorage:
    ...
    def load_nodes(self) -> Dict[str, KnowledgeNode]:
        """
        Загружает все узлы из базы данных.
        ...
        try:
            ...
            if not cursor.fetchone():
                logger.warning("Таблица nodes не существует, создаем структуру")
                self._initialize_database()
                conn.close()
                return {}
            
            cursor.execute("""
                SELECT id, name, description, node_type, domain, strength, timestamp, 
                       version, meta, spatial_info, temporal_info, history, contradictions, 
                       keyword_index, concept_index
                FROM nodes
            """)
            ...
            for row in cursor.fetchall():
                node = KnowledgeNode(
                    ...
                    spatial_info=json.loads(row[9]) if row[9] else {},
                    temporal_info=json.loads(row[10]) if row[10] else {}
                )
                
                # Дополнительные поля
                if len(row) > 11:
                    node.history = json.loads(row[11]) if row[11] else []
                    node.contradictions = json.loads(row[12]) if row[12] else []
                    node.keyword_index = json.loads(row[13]) if row[13] else {}
                    node.concept_index = json.loads(row[14]) if row[14] else {}
                nodes[node.id] = node
    ...
    def load_edges(self) -> Dict[str, KnowledgeEdge]:
        """
        Загружает все связи из базы данных.
        ...
        try:
            ...
            if not cursor.fetchone():
                logger.warning("Таблица edges не существует, создаем структуру")
                self._initialize_database()
                conn.close()
                return {}
            ...
            logger.debug(f"Доступные колонки в edges: {columns}")
            
            # Строим запрос в зависимости от доступных колонок
            if 'relation_type' in columns:
                cursor.execute("""
                    SELECT id, source_id, target_id, relation_type, strength, timestamp, 
                           version, meta, spatial_info, temporal_info
                    FROM edges
                """)
                ...
                for row in cursor.fetchall():
                    edge = KnowledgeEdge(
                        ...
                        spatial_info=json.loads(row[8]) if row[8] else {},
                        temporal_info=json.loads(row[9]) if row[9] else {}
                    )
                    edges[edge.id] = edge
            else:
                # Fallback если колонки не найдены
                logger.warning("Колонка relation_type не найдена, используем базовый запрос")
                cursor.execute("""
                    SELECT id, source_id, target_id, strength, timestamp
                    FROM edges
                """)
