...
"""Скрипт самопроверки CogniFlex — тестирует основные модули системы."""
...
logger = logging.getLogger("CogniFlexSelfTest")
...
# === Импорт ядра ===
try:
    from cogniflex.core.core_brain import CogniFlexBrain
except Exception as e:
    logger.error(f"Не удалось импортировать ядро CogniFlex: {e}")
    sys.exit(1)
...
def main():
    logger.info("=" * 60)
    logger.info(f"Запуск самопроверки CogniFlex — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info("=" * 60)

    brain = CogniFlexBrain()
    ...
    if brain.components.get("knowledge_graph"):
        for method in ["search_nodes", "search", "find_nodes", "query_nodes"]:
            if hasattr(brain.components["knowledge_graph"], method):
                test_component(f"KnowledgeGraph — {method}", getattr(brain.components["knowledge_graph"], method), "CogniFlex", 1)
                break

    if brain.components.get("memory_manager") and hasattr(brain.components["memory_manager"], "search"):
        test_component("MemoryManager — поиск", brain.components["memory_manager"].search, "CogniFlex")

    if brain.components.get("web_search_engine") and hasattr(brain.components["web_search_engine"], "search"):
        test_component("WebSearchEngine — поиск", brain.components["web_search_engine"].search, "CogniFlex", max_results=1)
    ...
    if hasattr(brain, "process_query"):
        test_component("Полный pipeline — process_query", brain.process_query, "Привет! Расскажи о системе CogniFlex.")
