...
from cogniflex.core.device_resolver import resolve_device, should_pin_memory
import logging
...
def default_collate(items: Sequence[Dict[str, Any]], pad_value: int = 0) -> Tuple[Dict[str, torch.Tensor], List[Meta]]:
    # Expect each item like {"input_ids": LongTensor[W], ... , "_meta": Meta}
    ...
    # Collate known keys; fall back to stacking if shapes equal
...
class TorchBatchAdapter:
    """
    Collects streaming items into optimal torch batches with bucketing, padding, and timeout triggers.

    Usage:
        adapter = TorchBatchAdapter(max_items=32, max_tokens=4096, timeout_ms=5)
        adapter.push(item)
    ...
    def __init__(
        ...
        # Optional event system for telemetry (CoreBrain.events or compatible)
        events: Optional[Any] = None,
        # Whitelist of allowed tensor keys for model consumption
        ...
        # Optional hook to prefetch and inject additional tensors (e.g., macroblocks)
        self._prefetch_fn = prefetch_fn
        # Telemetry event bus (may be None)
        self._events = events
        # Whitelist of keys accepted by the model; drop everything else to prevent leakage
    ...
    def push(self, item: Dict[str, Any] | BatchEnvelope) -> None:
        # Support receiving BatchEnvelope; unwrap at adapter ingress and emit telemetry
        ...
        # Sanitize item: keep only allowed tensor keys and _meta
    ...
    def try_pop_batch(self) -> Optional[Batch]:
        if not self._should_emit():
            return None
        # Determine how many items to emit respecting max_items and max_tokens
        ...
        if self.max_tokens is not None:
            ...
            if n_emit == 0:
                # Fallback to emit at least one item
                n_emit = 1
        items = self._buf[:n_emit]
        self._buf = self._buf[n_emit:]
        # Reset timer: if buffer still has items, restart; else clear
        self._t0 = self._now() if self._buf else None
        # Collate sanitized items only; _collate already expects clean keys
        tensors, metas = self._collate(items, self.pad_value)
        # Optional: merge prefetch outputs
            ...
            try:
                ...
                if isinstance(extra, dict):
                    ...
                    for k, v in extra.items():
                        if k in tensors:
                            # Suffix duplicate keys with "+prefetched" to preserve both
                            tensors[f"{k}+prefetched"] = v
                        else:
                            tensors[k] = v
            except Exception:
                # Never fail batch emission due to prefetch issues
                pass
        # Validate clean payload (no wrapper artefacts)
        try:
            assert_clean_batch(tensors)
        except AssertionError:
            # Drop any forbidden keys just in case (hardening)
            drop = {"_wrapper", "envelope_meta", "wrapper_meta"}
            for k in list(tensors.keys()):
                if k in drop:
                    tensors.pop(k, None)
        # Optionally pin memory for faster HtoD transfers on CUDA
            ...
            for k, v in tensors.items():
                if isinstance(v, torch.Tensor) and v.device.type == "cpu":
                    try:
                        ...
                        pinned[k] = v  # fallback without pinning
                else:
                    pinned[k] = v
            tensors = pinned
