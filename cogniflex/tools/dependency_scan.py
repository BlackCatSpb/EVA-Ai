...
# Detect project root (two levels up from this file: cogniflex/tools/ -> project root)
PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir, os.pardir))

# Primary package root to scan
PKG_ROOT = os.path.join(PROJECT_ROOT, 'cogniflex')
...
EXCLUDE_DIRS = {
    ...
    '.vscode', '.vs', '.pytest_cache', 'Помойка', 'cogniflex_models'
}
    ...
    for fn in filenames:
        if fn.endswith('.py'):
            ...
            if is_pkg:
                ...
                parts[-1] = parts[-1][:-3]
            mod = 'cogniflex' + ('.' + '.'.join([p for p in parts if p]) if parts else '')
            modules[fp] = mod
            if is_pkg:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            ...
            try:
                ...
                if target_mod:
                    if target_mod.startswith('cogniflex'):
