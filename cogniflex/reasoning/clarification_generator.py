...
class ClarificationGenerator:
    ...
    def generate_simple_clarification(self, query: str) -> List[str]:
        ...
        # Находим главные существительные
        nouns = self._extract_key_nouns(query)
        
        for noun in nouns:
            if 'дорог' in noun:
                questions.append("По какой дороге?")
            elif 'машин' in noun or 'автомобил' in noun:
                questions.append("Какая машина?")
            elif 'человек' in noun:
                questions.append("Кто этот человек?")
            elif 'город' in noun or 'посёлок' in noun:
                questions.append("В какой город?")
