...
logger = logging.getLogger("cogniflex.core.context_first_policy")
...
try:
    import psutil  # type: ignore
except (ImportError, ModuleNotFoundError, RuntimeError):  # psutil опционален
    psutil = None  # type: ignore
...
class ContextFirstPolicy:
    ...
    def _safe_set(self, key: str, value: Any) -> None:
        try:
            ...
            logger.debug(f"Error setting config key {key}: {e}")
            ...
            try:
                ...
                if hc and hasattr(hc, "enable_disk"):
                    try:
                        ...
                        logger.debug(f"Error enabling hybrid cache disk: {e}")
            except (AttributeError, RuntimeError, TypeError) as e:
                logger.debug(f"Error applying hybrid cache configuration: {e}")
