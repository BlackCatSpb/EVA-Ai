...
class MemoryGraphTrainer:
    ...
    def __init__(self, brain=None, config: Optional[TrainingConfig] = None):
        ...
        self.model = MemoryGraphNetwork(self.config).to(self.device)
    ...
    def _create_synthetic_data(self) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        num_nodes = 100
        node_features = torch.randn(num_nodes, self.config.embedding_dim)
        node_labels = torch.randint(0, 5, (num_nodes,))
        
        # Создаем случайные признаки связей
        num_edges = 200
        edge_features = torch.randn(num_edges, self.config.embedding_dim * 2)
        link_labels = torch.randint(0, 2, (num_edges,)).float()
    ...
    def _get_node_embedding(self, node) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if hasattr(node, 'embedding') and node.embedding is not None:
                return np.array(node.embedding)
            
            if hasattr(node, 'vector') and node.vector is not None:
                return np.array(node.vector)
        ...
        try:
            ...
            if self.brain and hasattr(self.brain, 'ml_unit') and self.brain.ml_unit:
                ...
                if text_processor and hasattr(text_processor, 'get_embeddings'):
                    embeddings = text_processor.get_embeddings([text])
    ...
    def _train_epoch(self, node_features, edge_features, link_labels, node_labels) -> Dict[str, float]:
        ...
        for batch_idx in range(num_batches):
            ...
            batch_nodes = node_features[start_idx:end_idx].to(self.device)
            batch_node_labels = node_labels[start_idx:end_idx].to(self.device)
            
            batch_edges = None
            batch_link_labels = None
            if edge_features is not None and link_labels is not None:
                if end_idx <= len(edge_features):
                    batch_edges = edge_features[start_idx:end_idx].to(self.device)
                    batch_link_labels = link_labels[start_idx:end_idx].to(self.device)
