"""
Модуль обнаружения и разрешения противоречий в системе CogniFlex
"""
...
def __getattr__(name):
    ...
    if name in imports:
        ...
        try:
            module = __import__(f'cogniflex.contradiction.{module_name}', fromlist=[class_name])
            return getattr(module, class_name)
        except (ImportError, AttributeError) as e:
            raise ImportError(f"Cannot import {name} from {module_name}: {e}")
    
    raise AttributeError(f"module 'cogniflex.contradiction' has no attribute '{name}'")
