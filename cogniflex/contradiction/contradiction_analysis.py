"""Модуль для анализа противоречий в системе CogniFlex"""
import os
import logging
import time
import json
...
logger = logging.getLogger("cogniflex.contradiction.analysis")
...
try:
    ...
    logger.warning("Scikit-learn недоступен. Будет использована альтернативная реализация cosine_similarity")
...
try:
    ...
    logger.warning("NLTK недоступен. Некоторые функции анализа будут ограничены")
...
def safe_cosine_similarity(vec1: np.ndarray, vec2: np.ndarray) -> float:
    """
    Вычисляет косинусное сходство с проверкой доступности библиотек
...
class ContradictionAnalysis:
    ...
    def _calculate_concept_importance(contradiction) -> float:
        ...
        sentiment_analyzer = SentimentIntensityAnalyzer() if NLTK_AVAILABLE else None
        stop_words = set()
        if NLTK_AVAILABLE:
        ...
        words = word_tokenize(fact_content) if NLTK_AVAILABLE else fact_content.split()
    ...
    def calculate_semantic_divergence(contradiction, nlp_model) -> float:
        ...
        # Вычисляем среднее косинусное расстояние
        total_distance = 0.0
        count = 0
        
        for i in range(len(embeddings)):
            for j in range(i + 1, len(embeddings)):
                distance = 1 - safe_cosine_similarity(embeddings[i], embeddings[j])
    ...
    def calculate_nlp_metrics(contradiction, nlp_model) -> Dict[str, Any]:
        ...
        try:
            ...
            if nlp_model and hasattr(nlp_model, 'encode'):
                try:
                    embeddings = nlp_model.encode(texts)
                    semantic_similarity = safe_cosine_similarity(embeddings[0], embeddings[1])
            ...
            if NLTK_AVAILABLE:
                try:
                    ...
                    metrics["sentiment_divergence"] = float(sentiment_divergence)
                except Exception as e:
