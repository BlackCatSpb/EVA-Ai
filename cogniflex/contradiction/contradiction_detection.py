...
class ContradictionDetector:
        ...
        if len(text_facts) > 1:
            ...
            for i in range(len(embeddings)):
                for j in range(i + 1, len(embeddings)):
                    similarity = cosine_similarity(np.array([embeddings[i]]), np.array([embeddings[j]]))[0][0]
                    if 1 - similarity > self.detection_threshold:
    ...
    def _calculate_text_divergence(self, text1: str, text2: str) -> float:
        ...
        try:
            ...
            embeddings = self.nlp_model.encode([text1, text2])
            
            # Вычисляем косинусное расстояние
            similarity = cosine_similarity(np.array([embeddings[0]]), np.array([embeddings[1]]))[0][0]
            
            # Преобразуем в расхождение
            divergence = 1.0 - similarity
            
            # Нормализуем (косинусное расстояние может быть отрицательным)
