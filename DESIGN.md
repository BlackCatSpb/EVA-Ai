...
- **CoreBrain** — координация через 9 миксинов (State, Query, Memory, Monitoring, Metrics, EventSubscription, CommandIssuer, ProcessTracker, Config)
- **Performance Optimizations** — 5 фаз оптимизации (LRU Cache, Async Web Search, Batch Processing, Streaming, Metrics System)
...
CoreBrain — тонкий координатор, использующий **9 миксинов** через множественное наследование:
...
CoreBrain(
    ...
    MetricsMixin,          # Performance metrics (latency, throughput, cache hit rate)
...
### 2.5.2 Performance Optimizations — Оптимизации производительности (5 фаз)

Система прошла комплексную оптимизацию производительности:

#### Phase 1: Caching & Timeouts
- **LRU Cache** для semantic_search (`FractalMemoryGraph._search_cache`)
  - maxsize: 100 entries
  - TTL: 300 seconds (5 минут)
  - Thread-safe реализация с RLock
  - **Результат**: 99% speedup для повторных запросов (~1ms vs 100-500ms)
- **Оптимизированные таймауты**:
  - condensed: 8s (было 30-60s)
  - extended: 20s (было 60s)
  - large: 45s (было 120s)
- **@timed декоратор** для мониторинга операций >50ms

#### Phase 2: Async Web Search
- **AsyncWebSearchClient** (`websearch/web_search_integrated.py`)
  - aiohttp вместо requests
  - Connection pooling: 10 connections, 5 per host
  - Keep-alive connections
- **Методы**: `search_async()`, `search_batch_async()`
- **Результат**: Неблокирующий веб-поиск, -100ms latency

#### Phase 3: Batch Processing
- **Batch векторизация** (`add_nodes_batch()`)
  - Размер батча: 32 (настраиваемо)
  - **Результат**: 12.4x ускорение (50 текстов за 0.37s vs 4.59s)
- **Batch поиск** (`semantic_search_batch()`)
  - Batch encoding всех запросов за один проход

#### Phase 4: Streaming & Async Pipeline
- **Streaming Generation** (`DualGenerator.generate_streaming()`)
  - Token-by-token generation
  - **Результат**: TTFT < 1s (time to first token)
- **AsyncGenerationPipeline** (`core/async_pipeline.py`)
...
- **Metrics System** (`core/metrics.py`)
  - Histogram (p50, p95, p99)
  - Counter (монотонно растущий)
  - Gauge (может изменяться)
  - Prometheus-compatible export
- **EVAMetrics** — предопределенные метрики:
  - `eva_request_duration_seconds`
  - `eva_generation_duration_seconds`
  - `eva_cache_hits_total` / `eva_cache_misses_total`
...
| POST | `/api/chat/stream` | **Streaming chat** (SSE, real-time tokens) |
...
| GET | `/api/metrics` | **Prometheus metrics** (CPU, memory, cache hit rate, latency) |
| GET | `/api/metrics?format=json` | **JSON format metrics** |
| GET | `/api/health` | **Health check** (basic) |
| GET | `/api/health/detailed` | **Health check** (detailed with system metrics) |
| GET | `/api/dashboard` | **Dashboard data** (charts, counters, gauges) |
...
│   │   ├── core_brain.py           # CoreBrain class (тонкий координатор, 9 миксинов)
...
│   │   ├── async_pipeline.py       # **AsyncGenerationPipeline** (streaming, priority queue)
│   │   ├── metrics.py              # **Metrics system** (histograms, counters, gauges)
...
│   │
│   ├── memory/
│   │   ├── fractal_graph_v2/       # **Fractal Graph V2** (SQLite + embeddings + clustering)
│   │   │   ├── __init__.py         # FractalMemoryGraph API (+ LRU Cache, batch methods)
│   │   │   ├── storage.py          # SQLite хранилище
│   │   │   ├── embeddings.py       # Sentence Transformers
│   │   │   ├── dual_generator.py   # **DualGenerator** (streaming, timeouts)
...
│   ├── storage/                    # Хранилище
│   ├── websearch/                  # Веб-поиск
│   │   └── web_search_integrated.py # **AsyncWebSearchClient** (aiohttp, connection pooling)
│   ├── models/                     # Модели
│   ├── config/                     # Конфигурация
│   ├── cache/                      # Кэши
│   ├── scripts/                    # Скрипты
...
| **3.0** | **2026-04-10** | **Performance Optimization (5 фаз)**: LRU Cache (99% speedup), Async Web Search, Batch Processing (12.4x), Streaming Generation, Async Pipeline, Metrics System, Health Checks, Prometheus export |
