    ...
    try:
        ...
        for i, text in enumerate(test_texts):
            if hasattr(processor, 'tokenize'):
                processor.tokenize(text, tokenization_callback, {"text": text, "index": i}, priority=5)
            else:
        ...
        for i, text in enumerate(large_texts):
            if hasattr(processor, 'tokenize'):
                processor.tokenize(text, perf_callback, {"index": i}, priority=3)
            else:
                tokens = processor.process_text(text)
        ...
        print(f"✓ Скорость: {avg_tokens * perf_completed / perf_time:.1f} токенов/сек")
...
def test_integration():
    ...
    try:
        ...
        for i, sentence in enumerate(test_sentences):
            if hasattr(processor, 'tokenize'):
                processor.tokenize(sentence, integration_callback, {"text": sentence, "index": i}, priority=7)
            else:
