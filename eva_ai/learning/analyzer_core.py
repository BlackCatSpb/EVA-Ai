"""Основной модуль самоанализа для ЕВА - ядро системы"""
import sqlite3
import os
import logging
import time
...
logger = logging.getLogger("eva_ai.analyzer_core")
...
class AnalyzerCore:
    """Основной интерфейс модуля самоанализа для ЕВА."""
    ...
    def __init__(self, brain=None, cache_dir: Optional[str] = None):
        ...
        """
        self.brain = brain
        self.cache_dir = cache_dir or os.path.join(os.path.dirname(__file__), "eva_self_analyzer_cache")
    ...
    def _save_data(self):
        ...
        try:
            ...
            cache_dir = os.path.join(os.path.dirname(__file__), '..', 'core', 'eva_cache', 'analytics')
