"""Модуль настроек для ЕВА GUI - полнофункциональная реализация"""
...
logger = logging.getLogger("eva_ai.gui.settings")

class SettingsModule:
    """Модуль для управления настройками интерфейса и системы ЕВА."""
    ...
    def _create_backup_settings_tab(self):
        ...
        try:
            ...
            self.vars['backup_dir'] = tk.StringVar(value=self.gui.settings.get("backup", {}).get("directory", os.path.expanduser("~/ЕВА_Backups")))
    ...
    def _create_manual_backup(self):
        ...
        try:
            ...
            backup_file = os.path.join(backup_dir, f"eva_backup_{timestamp}.json")
