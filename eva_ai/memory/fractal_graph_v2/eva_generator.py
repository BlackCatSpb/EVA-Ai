...
2. Виртуальные токены → маршрутизация через GGUFShadowProfiler
3. Генерация через RecursiveModelPipeline
4. Постобработка: замена виртуальных токенов на контент узлов
"""
...
class EVAGenerator:
    ...
    def __init__(
        ...
        logger.info("EVAGenerator инициализирован")
    ...
    def generate(self, request: GenerationRequest) -> GenerationResult:
        ...
        prompt = self._build_prompt(request.text, request.conversation_history, virtual_node_ids)
        reasoning_steps.append({
            'step': 5,
