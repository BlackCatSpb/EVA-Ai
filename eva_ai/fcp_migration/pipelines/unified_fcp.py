...
def test():
    logger.info("=" * 60)
    logger.info("Unified FCP Test - Multiple Queries")
    logger.info("=" * 60)
    ...
    queries = [
        ...
        # Medium (reasoning)  
        "Почему небо голубое?",
        "Как решить уравнение x+2=5?",
        
        # Complex (creative)
        "Напиши короткое стихотворение про нейросеть",
    ...
    for i, q in enumerate(queries):
        ...
        logger.info(f"---")
        logger.info(f"Tokens: {r['tokens']}, Domain: {r['domain']}, Layers: {r['layers']}")
        logger.info(f"Graph: {r['graph_nodes']}, TCM: {r['tcm_segments']}, Time: {r['time']}s")
        logger.info(f"Early Exit: {r['early_exit']}")
