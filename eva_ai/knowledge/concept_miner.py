...
RATIONALE: [обоснование]"""
    ...
    def _validate_candidate(self, candidate: PhantomCandidate):
        """
        Валидация кандидата (многоуровневая по спецификации):
        1. NLI-когерентность (deberta-v3-xsmall-mnli)
        2. Онтологическая совместимость
        3. Этическая фильтрация (6 принципов EVA)
    ...
    def _check_nli_coherence(self, context: str, candidate: PhantomCandidate) -> Dict:
        ...
        entailment → +0.25 к confidence
        neutral → передача на веб-поиск
        contradiction > 0.65 → отклонение
        """
...
{{"status": "entailment" | "neutral" | "contradiction", "confidence": 0.0-1.0, "reason": "краткое объяснение"}}"""
        ...
        try:
            ...
            for line in response.split('\n'):
                if line.startswith('{') and line.endswith('}'):
                    ...
                    elif status == "contradiction" and conf > 0.65:
                        pass
    ...
    def _check_ethics(self, candidate: PhantomCandidate) -> Dict:
        """Этическая фильтрация по 6 принципам EVA"""
...
В противном случае: {{"risk_level": "low", "block": false}}"""
    ...
    def _check_ontology_compliance(self, candidate: PhantomCandidate) -> Dict:
        ...
        try:
            ontology_rules_path = os.path.join(
                os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
                "eva_ai", "knowledge", "ontology_rules.json"
            )

            if not os.path.exists(ontology_rules_path):
                return {"compliant": True, "reason": "Онтологические правила не загружены"}

            with open(ontology_rules_path, 'r', encoding='utf-8') as f:
            ...
            for rule in rules:
                ...
                if rule_id == "hierarchy_violation":
                    pass
                elif rule_id == "node_type_combination":
                    pass
                elif rule_id == "semantic_density":
                    min_connections = rule.get('min_connections', 3)
            ...
            logger.warning(f"Ошибка онтологической проверки: {e}")
            return {"compliant": True, "reason": f"Ошибка проверки: {str(e)}"}
...
{{"verified": true/false, "updated_definition": "...", "confidence_delta": -0.2 to +0.4, "source_quality": "low|medium|high"}}"""
    ...
    def _update_lifecycle(self):
        """
        Обновление жизненного цикла концептов (по спецификации):
