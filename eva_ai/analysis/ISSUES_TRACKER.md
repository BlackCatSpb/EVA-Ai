...
| # | Проблема | Приоритет | Статус | Комментарий |
|---|----------|----------|--------|-------------|
| 3.3.1 | Подписан на `curator.*` которых нет | [КРИТ] | [ ] | GraphCurator не публикует события |
...
| 6.1.4 | Race conditions в UnifiedCacheBridge | [КРИТ] | [ ] | _load_state, save_state без locks |
| 6.1.5 | Nested deadlock | [КРИТ] | [ ] | _add_token_impl() → _save_token_to_disk() |
...
| # | Проблема | Приоритет | Статус | Комментарий |
|---|----------|----------|--------|-------------|
| 6.3.1 | CPU по умолчанию (init_factories.py:469) | [КРИТ] | [ ] | GPU не используется |
...
| 6.3.4 | Случайные векторы при fallback | [КРИТ] | [ ] | Ломает семантический поиск |
