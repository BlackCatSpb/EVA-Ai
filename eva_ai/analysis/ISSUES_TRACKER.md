...
| 5.3.2 | **3 версии OpenVINOGenerator** | [ВЫС] | [X] | ✅ Исправлено: только core/openvino_generator.py (hot_deployment удалён ранее) |
...
| 6.1.7 | 6 компонентов создают НОВЫЕ инстансы | [ВЫС] | [X] | ✅ Исправлено: HybridTokenCache.__new__ использует реестр, повторные вызовы возвращают тот же инстанс |
...
| 6.3.2 | hash() нестабилен между запусками | [ВЫС] | [X] | ✅ Проанализировано: in-memory кэши (lost on restart), persistent использует sha256 |
...
| 8.1.4 | Thinking tags leak to user | [КРИТ] | [X] | ✅ Исправлено: фильтрация <think></think> в response_generator.py (_postprocess_generated_text) и formatText() в app.js |
...
|---|---------|----------|--------|-------------|
| 11.1 | Distributed | `_init_distributed_system()` НЕ СУЩЕСТВУЕТ | [X] | ✅ Исправлено: удалён мёртвый импорт из eva_ai.distributed.database_utils в core_detection.py, код теперь использует sqlite3 напрямую |
| 11.2 | Training | `self.output_dir` НЕ определён | [X] | ✅ Исправлено: добавлен self.output_dir с значением по умолчанию в GGUFTrainingSystem.__init__ |
| 11.3 | GraphCurator | deferred_system не используется | [X] | ✅ Проанализировано: deferred_system УЖЕ используется (строки 129, 150, 237) - issue устарел |
...
| 12.8 | LRUCache | 2 | [P] | LRUCache (HybridTokenCache), LRUCacheWithTTL (FractalGraphV2) - разные функции (TTL), рефакторинг требует тестирования |
...
| 16.04.2026 | 5.2.1 ModelManagers | 27/68 | +1: removed 2 dead model managers; 5.3.2 OpenVINOGenerator; 6.3.2 hash analyzed |
| | | | |
