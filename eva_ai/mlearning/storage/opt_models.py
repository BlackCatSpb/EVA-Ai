"""Model management, loading, unloading for OptimizedFractalModelManager"""
...
def _clean_response(self, query: str, generated_text: str) -> str:
    ...
    response = response.replace('$$', '').replace('<pad>', '')
    response = ' '.join(response.split())
    
    try:
        from utils.text_quality import check_and_fix_response
