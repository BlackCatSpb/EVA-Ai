...
def get_qwen_model_path(model_size: str = "qwen2.5-0.5b") -> str:
    ...
    paths_to_check = [
        os.path.join(project_root, "eva_ai", "mlearning", "eva_models", model_size),
        os.path.join(project_root, "mlearning", "eva_models", model_size),
        os.path.join(os.getcwd(), "eva_ai", "mlearning", "eva_models", model_size),
        os.path.join(os.getcwd(), "mlearning", "eva_models", model_size),
    ]
...
class QwenModelManager:
    ...
    def __init__(
        ...
        self.cache_dir = cache_dir or os.path.join(os.getcwd(), "eva_cache", "qwen_models")
    ...
    def _initialize_model(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Не удалось загрузить generation_config ({e}), используем значения по умолчанию")
                self.generation_config = None
            except Exception as e:
                logger.warning(f"Не удалось загрузить generation_config: {e}")
                self.generation_config = None
