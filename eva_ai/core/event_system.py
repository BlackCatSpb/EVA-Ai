...
timeline_logger = logging.getLogger("eva_ai.events.timeline")
...
class ComponentInitializationManager:
    ...
    def __init__(self, event_bus: EventBus):
        ...
        self._logger = logging.getLogger("eva_ai.component_init")
        ...
        with self._initialization_locks[component_name]:
            ...
            try:
                ...
                if result is True or result is None:  # Успешная инициализация
                    ...
                    self._logger.info(f"Компонент {component_name} вернул False (опциональный компонент)")
                    self._initialized_components[component_name] = {
                        "instance": component_instance,
                        "initialized_at": time.time(),
                        "init_result": result,
...
class EventSystem:
    ...
    def __init__(self, event_bus=None, timeline_maxlen: int = 1000, enable_timeline: bool = True):
        ...
        self._logger = logging.getLogger("eva_ai.event_system")
