"""Модуль для управления системными метриками ЕВА"""
import time
import json
...
class SystemMetricsManager:
    ...
    def update_request_metrics(self, duration: float, success: bool):
        ...
        if len(self.request_durations) > 10000:
            self.request_durations = self.request_durations[-5000:]
    ...
    def record_event(self, data):
        """Регистрирует произвольное событие метрик."""
        try:
    ...
    def record_query_metrics(self, processing_time, success=True, query_length=None, response_length=None, tokens_processed=None):
        # Записываем метрики запроса
        self.update_request_metrics(processing_time, success)
        ...
        if query_length is not None and response_length is not None:
