"""Объединенный текстовый процессор с асинхронной токенизацией для ЕВА
...
# Import config
try:
    from eva.config import is_embedding_loading_disabled, DISABLE_ALL_MODELS
except ImportError:
...
try:
    from eva.core.base_component import BaseComponent
except ImportError:
    # Для обратной совместимости
    BaseComponent = object
    logger = logging.getLogger("eva.unified_text_processor")
else:
    logger = logging.getLogger("eva.unified_text_processor")
...
class UnifiedTextProcessor(BaseComponent):
    ...
    def __init__(self, brain: Any = None, config: Optional[Dict[str, Any]] = None):
        ...
        Args:
            brain: Ссылка на ядро ЕВА, которое предоставляет зависимости.
