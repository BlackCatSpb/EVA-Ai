...
class UnifiedFractalStorage:
    ...
    def _text_to_vector_fallback(self, text: str) -> Optional[torch.Tensor]:
        """Fallback text vectorization using simple hashing"""
        try:
            hash_val = hash(text) % 10000
            vector = torch.zeros(128, device=self.device)
            for i in range(128):
                vector[i] = (hash_val >> i) % 2
            return vector / vector.norm() + 1e-8
        except Exception as e:
            logger.warning(f"Text vectorization failed: {e}")
            return None
    ...
    def query_knowledge(self, query: Union[str, torch.Tensor],
        ...
        if isinstance(query, str):
            if not query or not self.node_vectors:
                return []
            query_vector = self._text_to_vector_fallback(query)
