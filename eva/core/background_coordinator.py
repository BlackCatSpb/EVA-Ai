...
logger = logging.getLogger("eva.core.autopilot")
timeline_logger = logging.getLogger("eva.core.autopilot.timeline")
...
class BackgroundCoordinator:
    ...
    def _run_loop(self) -> None:
        _idle_ticks = 0
        _min_interval = self.policies.tick_interval_s
        _max_interval = max(_min_interval * 10, 30.0)
        _backoff_factor = 2.0
        current_interval = _min_interval
        while True:
            with self._lock:
                if not self._running:
                    break
                ...
                try:
                    ...
                    if jobs_scheduled:
                        ...
                        current_interval = min(
                            _min_interval * (_backoff_factor ** min(_idle_ticks, 6)),
                            _max_interval,
                        )
                except Exception as e:
    ...
    def _tick(self) -> bool:
        """Возвращает True, если хотя бы одна задача была запланирована."""
        ...
        try:
            ...
            if self.cache:
                try:
                    ...
                    pass
            if job is None:
                raise ValueError(f"Job instance is None for {job_type_name}")
            job.run(context)
        except Exception as e:
