...
class HybridModelManager:
    ...
    def initialize(self) -> bool:
        ...
        try:
            with self._lock:
                ...
                model_path_config = "eva/mlearning/eva_models/qwen3.5-0.8b"
                qwen_path = os.path.join(project_root, model_path_config)
                
                # Если модель не найдена, пробуем альтернативные пути
                if not os.path.exists(qwen_path):
                    alt_paths = [
                        os.path.join(project_root, "eva", "mlearning", "eva_models", "qwen3.5-2b"),
                        os.path.join(project_root, "mlearning", "eva_models", "qwen3.5-0.8b"),
                        os.path.join(project_root, "mlearning", "eva_models", "qwen3.5-2b"),
                    ]
    ...
    def _analyze_resources(self):
        ...
        try:
            ...
            try:
                if os.path.exists("./eva_cache"):
                    # Используем shutil для кроссплатформенного получения размера диска
                    import shutil
                    self.free_ssd_gb = shutil.disk_usage("./eva_cache").free / 1024**3
    ...
    def _estimate_model_size(self, model_path: str) -> float:
        ...
        try:
            ...
            logger.warning(f"Ошибка оценки размера модели {model_path}: {e}")
            return 1.5
        except (ValueError, RuntimeError) as e:
            logger.warning(f"Ошибка расчета размера модели: {e}")
            return 1.5
    ...
    def _generate_fallback_response(self, prompt: str) -> str:
        ...
        fallbacks = {
            'привет': 'Привет! Я ЕВА, рада общению.',
            'здравств': 'Здравствуйте! Чем могу помочь?',
            'как дела': 'Спасибо, что спрашиваете! У меня всё хорошо.',
            'как тебя': 'Меня зовут ЕВА.',
            'кто ты': 'Я ЕВА - когнитивная AI система.',
        }
    ...
    def optimize_windows(self):
        ...
        try:
            with self._lock:
                usage_stats = {}
                for window_id, window in {**self.hot_windows, **self.warm_windows, **self.cold_windows}.items():
                    usage_stats[window_id] = window.access_count
                
                sorted_windows = sorted(usage_stats.items(), key=lambda x: x[1], reverse=True)
                hot_count = len(self.hot_windows)
                
                for idx, (window_id, _) in enumerate(sorted_windows):
                    if idx < hot_count and window_id in self.cold_windows:
                        window = self.cold_windows.pop(window_id)
                        self.hot_windows[window_id] = window
                    elif idx >= hot_count and idx < hot_count * 2 and window_id in self.hot_windows:
                        window = self.hot_windows.pop(window_id)
                        self.warm_windows[window_id] = window
                    elif idx >= hot_count * 2 and window_id in self.warm_windows:
                        window = self.warm_windows.pop(window_id)
                        self.cold_windows[window_id] = window
