...
class QueryMixin:
    ...
    def _handle_fg_only(self, query: str, user_context: Optional[Dict], start_time: float,
        ...
        """Handles queries using only FractalGraphV2 - no external models."""
        query_logger.info("FG-only mode: using FractalGraphV2 for generation")
        ...
        try:
            ...
            query_logger.debug(f"FG context: {len(context_results) if context_results else 0} chars")
        except Exception as e:
    ...
    def _generate_fg_response(self, query: str, context: Optional[str]) -> str:
        ...
        if '?' in query and any(w in query_lower for w in ['что', 'как', 'почему', 'зачем', 'когда', 'где', 'кто']):
            if context and len(context) > 50:
                return f"Из моей памяти:\n\n{context[:500]}"
            else:
                responses = [
                    "Интересный вопрос! К сожалению, у меня нет точной информации по этому запросу в текущей памяти.",
                    "Хороший вопрос. Для полного ответа мне нужно больше контекста.",
                    "Я обработала ваш запрос, но для детального ответа мне нужно изучить эту тему глубже."
                ]
        ...
        generic_responses = [
            "Я получила ваш запрос. Чтобы дать лучший ответ, позвольте мне обратиться к памяти.",
            "Интересная тема. Могу ли я уточнить, что именно вас интересует?",
            "Спасибо за ваш вопрос! Я обработала его с помощью фрактальной памяти."
        ]
