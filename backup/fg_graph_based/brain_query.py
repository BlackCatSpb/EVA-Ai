...
class QueryMixin:
    ...
    def _handle_fg_only(self, query: str, user_context: Optional[Dict], start_time: float,
        ...
        """Handles queries using only FractalGraphV2 - uses GraphBasedGenerator for proper generation."""
        query_logger.info("FG-only mode: using GraphBasedGenerator")
        ...
        try:
            from eva_ai.memory.fractal_graph_v2.test_generation import GraphBasedGenerator
            from eva_ai.memory.fractal_graph_v2.tokenizer import create_graph_tokenizer
            
            tokenizer = create_graph_tokenizer(fractal_graph)
            generator = GraphBasedGenerator(fractal_graph, tokenizer)
            result = generator.generate(query)
            
            if result and result.response:
                self._save_to_fractal_graph(query, result.response)
                return {
                    "response": result.response,
                    "text": result.response,
                    "status": "ok",
                    "confidence": result.confidence,
                    "source": "graph_based_generator",
                    "fallback_level": 0,
            ...
            query_logger.warning(f"GraphBasedGenerator error: {e}")
        
        context_results = None
        try:
            context_results = fractal_graph.get_context_for_query(query, max_length=512)
        except Exception as e:
            query_logger.debug(f"FG get_context_for_query error: {e}")
        
        if not context_results and hasattr(fractal_graph, 'semantic_search'):
            try:
                search_results = fractal_graph.semantic_search(query, top_k=5, min_similarity=0.3)
                if search_results:
                    context_parts = []
                ...
                query_logger.debug(f"FG semantic_search error: {e}")
        
        response_text = self._generate_fg_response(query, context_results)
