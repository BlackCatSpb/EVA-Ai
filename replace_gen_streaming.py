...
# Find the start of generate_streaming method
...
# Find the end of the method: next method at same indentation level (4 spaces)
# We'll look for '\n    def ' after start_pos, but not inside the method.
# Simple: find from start_pos+1 the next '\n    def ' that is not indented more.
# We'll search for pattern that matches newline + 4 spaces + 'def '
import re
pattern = r'\n    def '
matches = [m for m in re.finditer(pattern, content[start_pos+1:])]
if matches:
    # The first match is the next method
            ...
            def token_callback(token_text: str):
                ...
                while True:
                    if not in_thinking:
                        ...
                        if idx != -1:
                            # Текст до тега - это основной ответ
                            if idx > 0:
                        ...
                        idx = buffer.find("</think>")
                        if idx != -1:
                            # Текст до конца - это рассуждение
                            thinking_text_accum += buffer[:idx]
